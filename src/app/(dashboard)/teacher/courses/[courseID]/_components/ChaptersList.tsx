...
export default function ChaptersList({ onEdit, onReorder, items }: ChapterListProps) {
  const [isMounted, setIsMounted] = useState(false);
  const [chapters, setChapters] = useState(items);

  // TO FIX HYDRATION PROBLEM, AS DND IS NOT OPTIMIZED FOR SERVER-SIDE
  useEffect(() => {
    setIsMounted(true);
  }, []);

  // TO REHYDRATE, I DONT KNOW WHAT THIS IS FOR
  useEffect(() => {
    setChapters(items);
  }, [items]);
  ...
  const _handleOnDragEnd = (result: DropResult) => {
    ...
    // Chapters affected by the rearrangement
